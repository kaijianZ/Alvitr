...
class Daily(Downloader):
    ...
    def get_pids(self):
        ...
        return pattern.findall(daily_list_page.text), saveimg.mkdir(
            date=self.date)
        # return the pids
...
class Tag(Downloader):
    ...
    def get_pids(self):
        tag_list_page = requests.get(
            'https://www.pixiv.net/search.php?s_mode=s_tag_full&word=' + self.tag)
        pattern = re.compile('data-id="(\d*)"')
        # regular expression for pids of popular pics of specific tag

        return list(set(pattern.findall(tag_list_page.text))), saveimg.mkdir(
            tag=self.tag)
        # avoid duplicate pid
